...
- **Collections** (Firestore): `siteContent/main` (flat text keys), `templates` (→ web-experiences.html cards), `teasers` (→ cinematic-ads.html cards; same shape as templates minus `link`), `tags`. All sorted client-side by `order`, filtered by `published !== false`. index.html's Explore cards (WE_DATA) are static — not CMS-driven.
